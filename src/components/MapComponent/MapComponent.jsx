...
export default function MapComponent() {
  ...
  function findSafestRoute(routes, neighborhoods) {
    ...
    for (let i = 0; i < routes.length; i++) {
      ...
      if (!passesHighRisk) {
        console.log(
          `✅ Route ${i + 1} is safe: ${route.summary || "Unnamed Route"}`
        );
        return route;
      } else {
    ...
    console.log(
      "❌ No completely safe route found. Defaulting to the first route."
    );
    window.alert(
      "⚠️ Warning: No completely safe route available. Proceed carefully!"
    );
      ...
      (res, status) => {
        if (status === "OK") {
          ...
          const scoredRoutes = routes.map((route) => {
            const path = route.overview_path;
            const hotspotCount = neighborhoods.filter(
              (hotspot) =>
                hotspot.crimeCount > 600 &&
                path.some(
                  (pt) =>
  ...
  return (
    <div className="map-wrapper">
      ...
      <Chatbot
        mapCenter={
          selectedNeighborhood ? selectedNeighborhood.center : mapCenter
        }
        crimeCount={
          selectedNeighborhood ? selectedNeighborhood.crimeCount : null
        }
      />
