...
export default function MapComponent() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const pts = data.map((c) => {
        const { latitude: lat, longitude: lng } = toLatLon(
          +c.X,
          +c.Y,
          10,
          "N"
      ...
      const nbs = Object.entries(agg).map(([name, { count, types, points }]) => ({
        name,
        crimeCount: count,
        crimeTypes: types,
        center: {
  ...
  const getMarkerColor = (type) => {
    switch (type) {
      case "Mischief": return "orange";
      case "Other Theft": return "blue";
      case "Offence Against a Person": return "yellow";
      case "Homicide": return "purple";
      default: return "red";
    }
  };
    ...
    <div className="map-wrapper">
      <div className="map-controls">
        ...
        <div className="map-controls__starting">
          ...
          >
            {useCurrentLocation ? "Using ✓ Your Location" : "Use my current location"}
          </p>
        </div>
